...
const PublicationCard = ({
  ...
  const [isShowOverview, setIsShowOverview] =
  ...
  return (
    <Card className='flex gap-5 border border-neutral-300 px-6 py-4 dark:border-neutral-900'>
      ...
      <div className='w-4/5 space-y-3'>
        ...
        <button
          onClick={() => setIsShowOverview(!isShowOverview)}
          className='-ml-1 mt-5 flex items-center gap-1 text-sm text-neutral-500'
        >
          <HiChevronRight
            size={18}
            className={cn({
              'rotate-90 transition-all duration-300': isShowOverview,
            })}
          />
          {isShowOverview ? 'Hide' : 'Show'} Overview
        </button>
        <AnimatePresence>
          {isShowOverview && (
            <motion.ul
