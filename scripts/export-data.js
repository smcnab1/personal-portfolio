 ...
 *
 * This script exports all CMS data from the local SQLite database
 * to JSON files that can be imported into production.
 */
...
async function exportData() {
  try {
    ...
    const tables = {
      ...
      about: await prisma.about.findMany(),
      siteSettings: await prisma.siteSettings.findMany(),
      cmsUsers: await prisma.cMSUser.findMany(),
    ...
    Object.entries(tables).forEach(([table, data]) => {
      console.log(`${table.padEnd(15)}: ${data.length} records`);
    });
  } catch (error) {
...
async function importData() {
  try {
    ...
    if (!fs.existsSync(exportPath)) {
      throw new Error('Export file not found. Please upload the exports directory first.');
    }
    ...
    if (data.cmsUsers?.length > 0) {
      console.log('⚠️  CMS Users found in export. Skipping import for security.');
...
`;
  ...
  const exportDir = path.join(__dirname, 'exports');
  fs.writeFileSync(
    path.join(exportDir, 'import-to-production.js'),
    importScript,
  );
  console.log('📝 Generated import script: exports/import-to-production.js');
}
